...
function AddressAutocompleteComponent({
  ...
  // Base de données de rues de Québec
  const streets = [
    // Boulevards de Laval
    ...
    // Autoroutes et chemins
    'Autoroute Laval Ouest',
    'Autoroute Laval Est',
  ...
  return (
    <div ref={wrapperRef} className="relative w-full">
      ...
      {showSuggestions && inputValue.length > 0 && suggestions.length === 0 && !isSearching && parseAddress(inputValue).civicNumber && (
        <div className="absolute z-50 w-full mt-1 bg-white border border-[#CCCCCC] rounded-lg shadow-lg p-4 text-center">
          <p className="text-[#666666] text-sm">
            Aucune rue trouvée. Continuez à taper pour affiner la recherche.
          </p>
        </div>
      )}
